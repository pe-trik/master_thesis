...
\label{read-newstest}
\XXX{To the best of our knowledge, there is no SLT test set from Czech to English.} We therefore decided to create one.
...
The re-speaker was instructed to use our online tool \texttt{Voice Recorder}\footnote{We developed this tool within the ELITR project (\url{elitr.eu}). \XXX{Can I make the GitHub repository public?} Source code of the recorder is available at \url{https://github.com/ELITR/voice-recorder}}. The interface displays one sentence at time. The user can read the sentence, record it and replay. If she or he is not satisfied with the recording, the re-speaker may record a new version. With this workflow, the sentences are already segmented.
...
\begin{table}[t]
	...
	\begin{tabular}{lc|c}
		...
		& graphemes      & 18.60 \% WER 
	\end{tabular}
	\caption{Evaluation of acoustic models on our test set derived from \texttt{news\-test\-2018}.}
