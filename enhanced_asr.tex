...
\XXX{The BPE vocabulary of the \textit{clean} setup uses clean filtered CzEng. The \textit{corrupted} setup utilizes data obtained from acoustic models. We describe the procedure of obtaining these data in \cref{sec:asr_corrupted} on page \pageref{sec:asr_corrupted}. } We have approximately 7M corrupted sentences. For \textit{mixed data} setup, we take a random subset of 7M from clean filtered CzEng. We do not take the whole CzEng as it has 57M sentence pairs. 
...
First, we are interested in model \XXX{(note, the Transformer model, not the BPE encoding ehre we decided to use clean data in all our experiments)} performance when trained on clean data. In this chapter, we use clean phonemized CzEng. In \cref{chap:fine_tune_enhanced} we further study fine-tuning of models for corrupted ASR data.
